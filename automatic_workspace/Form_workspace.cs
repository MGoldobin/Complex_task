...
namespace automatic_workspace
{
    ...
    {
        private readonly string connect_string = Properties.Settings.Default.dataString;
        ...
        public Form_workspace()
        {
            ...
            } //права для операторов и админов
        ...
        private void InitializeQuestions()
        {
            ...
            using (var connect = new NpgsqlConnection(connect_string))
            {
                ...
                while (reader.Read())
                {
                    questions.Rows.Add(reader["id_question"], reader["link_id"], reader["count_view"],
                        reader["subject_id"], reader["s_name"], reader["user_id"], reader["login"],
                        reader["age"], reader["status_id"], reader["st_name"]); //fill data_grid_view
                }
            }
        ...
        private void InitializeOperators()
        {
            ...
            connect.Open();
            var reader = connect.ExecuteReader("select id_user, login, password, is_admin from operator_workspace");
            while (reader.Read())
            {
            ...
            if (data_grid_view_operators.IsCurrentRowDirty)
                if (!e.Cancel)
                {
                    ...
                    if (row.Tag == null) //this is new row
                    {
                        ...
                        var connect = new NpgsqlConnection(connect_string);
                        connect.Open();
                        id_user = (int?)connect.ExecuteScalar("insert into operator_workspace(login, password, is_admin) values (@login, @password, @is_admin) returning id_user",
                    ...
                    else
                    {
                        ...
                        {
                            command = "update operator_workspace set login = @login, is_admin = @is_admin where login=@old_login";
                            connect.Execute(command, new { login, is_admin, old_login });
                        }
                        else
                        {
                            string password = Hash.HashMD5(row.Cells["password"].Value.ToString() + Hash.salt);
                            connect.Execute("update operator_workspace set login=@login, password=@password, is_admin=@is_admin where login = @old_login",
            ...
            if (!row.IsNewRow)
            {
                if (!e.Cancel)
                {
                    ...
                    if (questions.IsCurrentRowDirty)
                    {
                        ...
                        var command_status = new NpgsqlCommand() { Connection = connect };
                        ...
                        var command_check = new NpgsqlCommand() { Connection = connect };
                        command_check.CommandText = "select id_user from users where login = @login";
                        ...
                        var command_user = new NpgsqlCommand() { Connection = connect };
                        ...
                        var command_for_questions = new NpgsqlCommand() { Connection = connect };
        ...
        private void func_update_user(NpgsqlConnection connection, NpgsqlCommand command_user, int? id_user, object id_question, object login)
        {
            ...
            var command_update_questions = new NpgsqlCommand() { Connection = connection };
            ...
            if (!e.Cancel)
            {
                if (data.IsCurrentRowDirty)
                {
                    ...
                    if (row.Tag == null)
                    {
                        ...
                        var command = new NpgsqlCommand() { Connection = connect, CommandText = res.Sql };
        ...
        private void Delete_one_row(DataGridViewRow row)
        {
            ...
            {
                connect.Execute("delete from operator_workspace where id_user = @id", new { id });
            }
            ...
            if (comboBox_query.Text != null)
            {
                ...
                var query = new PostgresCompiler().Compile(new Query("questions")
                    ...
                    new Query("statuses")
                        ...
                        .AsCount());
                var command = new NpgsqlCommand() { Connection = connect, CommandText = query.Sql };
