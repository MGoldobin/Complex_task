...
namespace automatic_workspace
{
    public partial class Form_auth : Form
    {
        ...
        {
            bool result;
            OperatorWorkspace user = db.OperatorWorkspace.Where(op => op.Login == login).FirstOrDefault();
            if (user != null)
            {
        ...
        private void button_registration_Click(object sender, EventArgs e)
        {
            ...
            {
                ExecuteAdd_registration(textbox_login.Text, Hash.HashMD5(textbox_password.Text));
        ...
        {
            db.OperatorWorkspace.Add(new OperatorWorkspace(login, password));
            try
            {
                db.SaveChanges();
        ...
        private void button_back_Click(object sender, EventArgs e)
        {
            if (Info_operate.Current_operation == Info_operate.Types.add_from_auth)
            {
                Info_operate.Current_operation = Info_operate.Types.add;
                Close();
            }
        }
    }
